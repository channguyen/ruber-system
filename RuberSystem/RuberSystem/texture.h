...
private:
	void load_multiple_textures_from_image(vector<pair<image*, image*> >& images, 
		...
		gl_gen_textures(no_textures, texture_ids);
		for (unsigned i = 0; i < no_textures; ++i) {
			gl_bind_texture(GL_TEXTURE_2D, texture_ids[i]);
			if (types[i] == TEXTURE_REGULAR) {
				gl_tex_image_2d(
					GL_TEXTURE_2D,
					0,
			...
			hm.insert(make_pair<string, unsigned>(filenames[i].first, texture_ids[i]));	
		}
		gl_delete_textures(no_textures, (GLuint* )&texture_ids);
		delete []texture_ids;
	}

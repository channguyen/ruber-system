...
public:
	...
	virtual void draw_bounding_sphere() {
		gl_push_matrix(); {
			...
			glut_wire_sphere(bounding_sphere_radius, 40, 17);
		} gl_pop_matrix();
	}
	...
	void update_internal() {
		...
		gl_push_matrix(); {
			...
			if (rotate_about == false) { 
				...
				gl_rotate_f(roll, 0, 0, 1);
				gl_rotate_f(yaw, 0, 1, 0);
				gl_rotate_f(pitch, 1, 0, 0);
			} else {
				gl_rotate_f(roll, 0, 0, 1);
				gl_rotate_f(yaw, 0, 1, 0);
				gl_rotate_f(pitch, 1, 0, 0);
	...
	void update_smart_movement(const vector3<float> &target) {
		...
		gl_push_matrix(); {
			gl_load_identity();
			gl_rotate_f(1.0f, axis.get_x(), axis.get_y(), axis.get_z());
			gl_translate_f(position.get_x(), position.get_y(), position.get_z());
			gl_get_float_v(GL_MODELVIEW_MATRIX, OM);
		} gl_pop_matrix();
	}
	...
	void draw_axes() {
		...
		gl_begin(GL_LINES); {
			// draw x
			gl_color_3fv(get_color(colors::color_name::red));
			gl_vertex_3f(0.0, 0.0, 0.0);
			gl_vertex_3f(AXIS_LENGTH, 0.0, 0.0);

			// draw y
			gl_color_3fv(get_color(colors::color_name::green));
			gl_vertex_3f(0.0, 0.0, 0.0);
			gl_vertex_3f(0.0, AXIS_LENGTH, 0.0);

			// draw z
			gl_color_3fv(get_color(colors::color_name::blue));
			gl_vertex_3f(0.0, 0.0, 0.0);
			gl_vertex_3f(0.0, 0.0, AXIS_LENGTH);
		} gl_end();
	}
	...
	void draw_vector(vector3<float> &v) {
		v.normalize();
		gl_begin(GL_LINES); {
			gl_color_3fv(get_color(colors::color_name::feldspar));
			gl_vertex_3f(0.0, 0.0, 0.0);
			gl_vertex_3f(10000 * v.get_x(), 10000 * v.get_y(), 10000 * v.get_z());
		} gl_end();
	}

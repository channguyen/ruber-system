...
public:
	...
	static const int VERTICAL_TEXT_OFFSET = 20;
	static const int GRAVITY = 90000000;
	static const int GRAVITY_THRESHOLD = 1800;
	static const int oo = 1000000000;
	static const int TRACKING_FRAME = 50;
...
public:
	...
	void set_viewing_volume(const viewing_mode &m, int w, int h) const {
		...
		gl_load_identity();
		if (m == PERSPECTIVE) {
			glu_perspective(view_angle, (float)w/h, 10.0, 900000.0);
		} else {
			gl_ortho(-w/2, w, -h/2, h, -1.0, 1.0);
		}
	}
	...
	void draw_game_status() {
		...
		gl_raster_pos_3f(-15000.0, 15000.0, 0.0);
		if (is_player_lose()) {
			...
			if (m->scan_target(apollo) && apollo->is_alive()) {
				if (m->get_name() == "U.Missile") {
					if (unum_smart_torpedo->is_alive() == false) {
						...
						sound::play_sound_missile_shooting();
						// debug
	...
	void on_special_key(int key, int x, int y) {
		...
		if (glut_get_modifiers() == GLUT_ACTIVE_CTRL && key == GLUT_KEY_UP) {
			apollo->turn(spaceship::direction::UP);
		} else if (glut_get_modifiers() == GLUT_ACTIVE_CTRL && key == GLUT_KEY_DOWN) {
			apollo->turn(spaceship::direction::DOWN);
		} else if (glut_get_modifiers() == GLUT_ACTIVE_CTRL && key == GLUT_KEY_LEFT) {
			apollo->turn(spaceship::direction::BACKWARD);
		} else if (glut_get_modifiers() == GLUT_ACTIVE_CTRL && key == GLUT_KEY_RIGHT) {
			apollo->turn(spaceship::direction::FORWARD);
		} else if (glut_get_modifiers() == GLUT_ACTIVE_SHIFT && key == GLUT_KEY_UP) {
			led->turn(light_direction::UP);
		} else if (glut_get_modifiers() == GLUT_ACTIVE_SHIFT && key == GLUT_KEY_DOWN) {
			led->turn(light_direction::DOWN);
		} else if (glut_get_modifiers() == GLUT_ACTIVE_SHIFT && key == GLUT_KEY_LEFT) {
			led->turn(light_direction::LEFT);
		} else if (glut_get_modifiers() == GLUT_ACTIVE_SHIFT && key == GLUT_KEY_RIGHT) {
...
private:
	...
	void draw_galaxy_skybox(int length) {
		...
		gl_bind_texture(GL_TEXTURE_2D, space_textures[st_idx]);
		...
		// gl_color_3fv(get_color(white));
		gl_begin(GL_QUADS); {
			// Negative X
			{
				gl_tex_coord_2f(0, 0);
				gl_vertex_3f(-length, -length, length);
				gl_tex_coord_2f(1, 0);
				gl_vertex_3f(-length, -length, -length);
				gl_tex_coord_2f(1, 1);
				gl_vertex_3f(-length, length, -length);
	...
	void draw_text(const string &text, int x, int y, int z) const {
		gl_raster_pos_3i(x, y, z);  
		gl_call_lists(text.length(), GL_BYTE, (char *)text.c_str());
	}

	void write_bitmap_string(void *font, char *str) {  
		for (char *c = str; *c != '\0'; c++) {
			glut_bitmap_character(font, *c);
		}
	}
...
private:
	void setup_text_window(float r, float g, float b, float a) const {
		...
		// set light materials
		gl_light_model_fv(GL_LIGHT_MODEL_AMBIENT, ambient);
		gl_light_fv(GL_LIGHT0, GL_DIFFUSE, diffuse);
		gl_light_fv(GL_LIGHT0, GL_POSITION, position);
		gl_enable(GL_LIGHTING);
		gl_enable(GL_LIGHT0);
	...
	void setup_cameras() {
		using namespace galaxy_constants;
		namespace cv = galaxy_constants::camera_view;
		float planet_up[3] = {1, 0, 0};

		cameras.push_back(new camera("front", cv::front::position, cv::front::lookat, cv::front::up));
		cameras.push_back(new camera("top", cv::top::position, cv::top::lookat, cv::top::up));
		cameras.push_back(new camera("ship", cv::ship::position, cv::ship::lookat, cv::ship::up));
		cameras.push_back(new camera("following", cv::ship::position, cv::ship::lookat, cv::ship::up));

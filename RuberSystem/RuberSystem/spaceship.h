...
private:
	void draw_model() {
		gl_call_list(list_id);
		if (bounding_sphere) {
			gl_color_3fv(get_color(colors::white));
			glut_wire_sphere(bounding_sphere_radius/100.0, 40, 17);
		}
	}

	void draw_explosion() {
		if (explosion_radius > 0) {
			gl_color_3fv(get_color(colors::red));
			glut_wire_sphere(explosion_radius, 30, 17);
			explosion_radius -= 50;
		} else {
			gl_color_3fv(get_color(colors::white));
			glut_wire_sphere(50, 30, 17);
		}
	}

	void draw_spaceship() {
		// draw them
		gl_color_3fv(get_color(base_color));
		if (is_solid()) {
			glut_solid_cone(base, height, 30, 17);
		} else {
			glut_wire_cone(base, height, 30, 17);
		}
		gl_translate_f(0, base/2, height/2);
		gl_color_3fv(get_color(top_color));
		if (solid) {
			glut_solid_sphere(base/2, 30, 17);
		} else {
			glut_wire_sphere(base/2, 30, 17);
		}
		// draw tail
		if (moving) {
			gl_translate_f(0, -80, -60);
			gl_color_3fv(get_color(colors::fire_brick));
			glut_solid_cube(40);
			sound::play_spaceship_moving();
		}
	}
